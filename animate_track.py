...
# Animation erstellen
def create_animation(points, output_file='track_animation.gif', duration=5, frame_skip=1, line_color='b', line_width=2, glow_width_factor=3, glow_alpha=0.3):
    ...
    # Linie für den Track    
    glow1, = ax.plot([], [], '-', color=line_color, linewidth=line_width * 4, alpha=0.1)
    glow2, = ax.plot([], [], '-', color=line_color, linewidth=line_width * 3.5, alpha=0.15)
    glow3, = ax.plot([], [], '-', color=line_color, linewidth=line_width * 3, alpha=0.2)
    glow4, = ax.plot([], [], '-', color=line_color, linewidth=line_width * 2.5, alpha=0.25)
    glow5, = ax.plot([], [], '-', color=line_color, linewidth=line_width * 2, alpha=0.3)
    ...
    # Animation berechnen
    print(f"{len(points)} Punkte, Frames werden alle {frame_skip} Punkte gespeichert...")
...
if __name__ == '__main__':
    ...
    print(f"Gefunden: {len(points)} Trackpunkte")
    
    print("Erstelle Animation...")
    frame_skip = 10  # Jeder 10. Frame wird gespeichert
    create_animation(points, output_file='track_animation.gif', duration=5, frame_skip=frame_skip, line_color='#FC4C02', line_width=4, glow_width_factor=3, glow_alpha=0.3)
